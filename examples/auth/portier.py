"""
This package is used to authenticate users, using an external portier broker:
    https://portier.github.io/

The code in this module is similar to the reference implementation:
    https://github.com/portier/demo-rp

Some changes were needed to make it work async.
...
from api.auth import generate_auth_tokens

bp = Blueprint('portier')
...
async def discover_keys(
    ...
    """
    # Check the cache
    cache_key = f'jwks:{broker_url}'
    raw_jwks = await redis.get(key=cache_key)

    if not raw_jwks:
        # Fetch Discovery Document
        discovery = await get_json(
            url=f'{broker_url}/.well-known/openid-configuration',
        )
...
async def get_verified_email(
    ...
    if not await redis.delete(key=payload['nonce']):
...
@bp.get('/portier-login')
async def login_get(req):
    return response.text('portier login GET')


@bp.post('/portier-login')
async def login_post(req):
    """
    <form class="demo" action="http://localhost:8000/portier-login" method="POST">
    ...
    # Store a nonce for this authentication request.
    nonce = uuid4().hex

    # Save nonce for portier validation.
    await redis.set(
        key=nonce,
        value=f'{cfg.PORTIER_WEBSITE_URL}/portier-verify',
        expire=150,
    )
    ...
    args = {
        ...
        'client_id': cfg.PORTIER_WEBSITE_URL,
        'redirect_uri': f'{cfg.PORTIER_WEBSITE_URL}/portier-verify',
    }

    query_args = urlencode(args)

    # FIXME: The Authorization Endpoint should be discovered, not hardcoded.
    url = cfg.PORTIER_BROKER_URL + '/auth?' + query_args

    return response.redirect(url)


@bp.post('/portier-verify')
async def verify_post(req):
    """
    Post your email authentication request to this route.
    This will post a request to the external portier broker.
    The portier broker returns a token.
    If the token from the portier broker is valid, we create our own 'user'
    token, that contains some user data.
    """
    cfg = req.app.config
    redis = req.app.redis
    ...
    if 'error' in combined_data:
        err = combined_data.get('error')
        desc = combined_data.get('error_description') or ''
        msg = f'Broker Error ({err}): {desc}'

        return response.json({'error': msg}, status=400)

    # Get the user's signed identity token from the HTTP POST form data.
    token = req.form.get('id_token')

    # Check the validity and authenticity of the identity token
    try:
        email = await get_verified_email(
            token=token,
            website_url=cfg.PORTIER_WEBSITE_URL,
            broker_url=cfg.PORTIER_BROKER_URL,
            redis=redis,
        )
    except RuntimeError as exc:
        return response.json({'error': exc}, status=400)

    # Generate default API auth tokens.
    return generate_auth_tokens(email, cfg)
